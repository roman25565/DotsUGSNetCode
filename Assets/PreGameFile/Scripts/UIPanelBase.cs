...
public class UIPanelBase : MonoBehaviour
{
    ...
    MainMenuUIManager m_gameManager;
    CanvasGroup m_canvasGroup;

    List<UIPanelBase>
        m_uiPanelsInChildren =
            new List<UIPanelBase>(); // Otherwise, when this Shows/Hides, the children won't know to update their own visibility.


    protected MainMenuUIManager MainMenuManager
    {
        get
        {
            if (m_gameManager != null) return m_gameManager;
            return m_gameManager = MainMenuUIManager.Instance;
        }
    }
    ...
    {
        var children =
            GetComponentsInChildren<UIPanelBase>(
                true); // Note that this won't detect children in GameObjects added during gameplay, if there were any.
